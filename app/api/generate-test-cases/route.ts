...
async function getRepoTree({
  ...
  if (!res.ok) {
    if (res.status === 401) {
      ...
      throw { isAuthError: true, response }; // bubble up to POST handler
    }
...
async function readGithubFile({
  ...
  if (!res.ok) {
    if (res.status === 401) {
      ...
      response.cookies.delete("gh_token");
      throw { isAuthError: true, response };
    }
    return null; // for other errors (404, 403, etc.) just skip the file
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[generate-test-cases] body:", { userId, repoId, owner, repo, branch })
    console.log("[generate-test-cases] githubToken present:", !!githubToken)
  ...
  console.log("[generate-test-cases] repoFiles found:", repoFiles.length)
  ...
  repoFiles.map((file: any) =>
    readGithubFile({
      ...
      if (err?.isAuthError) throw err; // re-throw so catch block sees it
      return null; // swallow other errors
    })
  )
);
    ...
    if (validFiles.length === 0) {
      console.log("[generate-test-cases] no valid files found")
      return NextResponse.json(
        {
...
`;
    ...
    if (!testCases.length) {
      console.log("[generate-test-cases] gemini returned no test cases")
      return NextResponse.json(
        {
    ...
    return error.response; // ← must return, not fall through
  }
